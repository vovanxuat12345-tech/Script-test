...
bugReportFrame.Size = UDim2.new(0, 320, 0, 140); bugReportFrame.Position = UDim2.new(0.5, -160, 0.5, -70); bugReportFrame.BackgroundColor3 = Color3.fromRGB(10, 10, 10); bugReportFrame.BackgroundTransparency = 0.25; bugReportFrame.Visible = false; bugReportFrame.ZIndex = 100
Instance.new("UICorner", bugReportFrame).CornerRadius = UDim.new(0, 8)

local stroke = Instance.new("UIStroke", bugReportFrame)
stroke.Color = Color3.new(1, 0, 0); stroke.Thickness = 2; stroke.ApplyStrokeMode = Enum.ApplyStrokeMode.Border

local bugTextLabel = Instance.new("TextLabel", bugReportFrame)
bugTextLabel.Size = UDim2.new(1, -20, 1, -20); bugTextLabel.Position = UDim2.new(0, 10, 0, 10); bugTextLabel.BackgroundTransparency = 1; bugTextLabel.Text = "I'm sorry, but there are still a few bugs that I haven't been able to fix yet, such as at stage 243. After all, I'm truly sorry everyone."; bugTextLabel.TextColor3 = Color3.new(1, 1, 1); bugTextLabel.Font = Enum.Font.GothamMedium; bugTextLabel.TextSize = 14; bugTextLabel.TextWrapped = true; bugTextLabel.ZIndex = 101
...
-- ========================================================
-- BIẾN TOÀN CỤC PHỤC VỤ CƠ CHẾ CHỐNG HỤT STAGE (-1 STAGE)
-- ========================================================
local currentTargetStageNum = -1 
local isStuckRollback = false    
...
local function flyToTarget(target)
    ...
    while running and not touched and not isHandling243 and not isStuckRollback and hrp.Position.Y < upY - 1 do 
        hrp.Velocity = Vector3.new(0, 50, 0); task.wait() 
    end
    
    while running and not touched and not isHandling243 and not isStuckRollback and (Vector2.new(hrp.Position.X, hrp.Position.Z) - Vector2.new(targetPos.X, targetPos.Z)).Magnitude > 4 do
        hrp.Velocity = (Vector3.new(targetPos.X, hrp.Position.Y, targetPos.Z) - hrp.Position).Unit * currentFlySpeed
        task.wait()
    end
    
    local t = tick()
    while running and not touched and not isHandling243 and not isStuckRollback do
    ...
    if not isHandling243 and not isStuckRollback then hrp.Velocity = Vector3.new(0, 0.2, 0) end
    if conn then conn:Disconnect(); conn = nil end 
    
    currentFlySpeed = DEFAULT_SETTINGS.FLY_SPEED
    scanMultiplier = 1
    
    if not isStuckRollback then
...
local function startFarming()
    ...
    task.spawn(function()
        while running do
            task.wait(0.1) -- Quét liên tục kiểm tra vị trí
            
            local char = player.Character
            local hrp = char and char:FindFirstChild("HumanoidRootPart")
            
            -- Nếu đang nhắm mục tiêu hợp lệ (> 1) và không trong chuỗi xử lý đặc biệt 243
            if hrp and currentTargetStageNum > 1 and not isHandling243 and not isStuckRollback then
                -- Tìm vị trí checkpoint cũ đứng ngay phía trước (currentTargetStageNum - 1)
                ...
                if previousStage and previousStage:IsA("BasePart") then
                    -- Nếu khoảng cách của nhân vật đột ngột cực gần checkpoint cũ (< 8 studs) 
                    -- trong khi đáng lẽ ra đang phải bay tới checkpoint mới -> Game vừa kéo giật lùi mình về (Hụt stage!)
                    if (hrp.Position - previousStage.Position).Magnitude < 8 then
                        isStuckRollback = true -- Kích hoạt cờ báo hiệu lùi stage, ngắt luồng bay hiện tại
                        
                        -- Thực hiện trừ đi 1 stage mục tiêu để hệ thống tính toán lại
                        currentTargetStageNum = currentTargetStageNum - 1 
                        
                        task.wait(0.3) -- Chờ nhẹ cho trạng thái ổn định lại
                        isStuckRollback = false -- Nhả cờ để tiếp tục vòng lặp farm mới
                    end
                end
            end
        end
    ...
    task.spawn(function()
        ...
        while running do
            if isHandling243 or isStuckRollback then task.wait() continue end 
            
            local hrp = player.Character and player.Character:FindFirstChild("HumanoidRootPart")
            if not hrp then task.wait(1) continue end
            
            -- Chỉ quét tìm vị trí gần nhất khi hệ thống không trong trạng thái đang xử lý lùi stage
            ...
            -- Cập nhật Stage mục tiêu tiếp theo vào biến chung để luồng check rollback kiểm soát
            currentTargetStageNum = closestStageNum + 1
            
            local nextTarget = DEFAULT_SETTINGS.CHECKPOINT_FOLDER:FindFirstChild(tostring(currentTargetStageNum))
...
player.CharacterAdded:Connect(function() 
    running = false; isHandling243 = false; isStuckRollback = false; toggleProtection(false)
    currentFlySpeed = DEFAULT_SETTINGS.FLY_SPEED
    scanMultiplier = 1; currentTargetStageNum = -1
